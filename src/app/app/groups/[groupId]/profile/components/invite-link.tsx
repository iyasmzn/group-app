...
export default function InviteLink() {
  ...
  const handleOpen = async () => {
    try {
      ...
      if (errRole || !role) {
        toast.error(
          errRole?.message || "There is no member role. Can't share group via share link."
        )
        return
      }

      await createInvite(groupId, role.id)
        .then((res) => {
          setInvite(res)
          setOpen(true)
        })
  ...
  return (
    <>
      ...
      <button
        className="flex flex-col py-2 px-3 rounded-xl items-center border border-primary hover:bg-secondary transition-all"
        onClick={handleOpen}
      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <Card>
            <CardContent className="flex items-center gap-3">
              ...
              <Button
                ...
                className="flex-1"
                onClick={() => invite && handleCopy(invite.code)}
              >
                <Copy /> Copy Code
              </Button>
            </CardContent>
          ...
          <p className="text-center text-secondary-foreground text-sm">
            Expires at: {invite && invite.expires_at && longDate(invite.expires_at)}
          </p>
