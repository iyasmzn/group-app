...
export default function DescriptionCard({ group }: DescriptionCardProps) {
  ...
  useEffect(() => {
    ...
    if (group.desc_updatedby) setDescriptionUpdatedby(group.desc_updatedby.full_name || null)
  }, [group])
  ...
  const updateDescription = async () => {
    ...
    await update(group.id, {
      ...
      .catch((error) => {
        ...
        setDescriptionUpdatedat(group?.description_updatedat)
        if (group && group.desc_updatedby)
          setDescriptionUpdatedby(group?.desc_updatedby?.full_name || null)
      })
      .finally(() => {
        setLoading(false)
      })
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Enter the description of the group you want.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="grid grid-cols-2 gap-2">
            ...
            <Button disabled={!description} onClick={() => updateDescription()}>
              Save changes
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Card className="mb-4 relative overflow-hidden">
        ...
        <CardContent>
          ...
          {loading && <Skeleton className="h-6 w-70"></Skeleton>}
          ...
          {descriptionUpdatedat && (
            <CardDescription className="flex flex-col items-end">
              {loading ? (
                ...
                <>
                  <span>updated at {longDateTime(descriptionUpdatedat)}</span>
                  <span>by {descriptionUpdatedby}</span>
                </>
              )}
            </CardDescription>
          )}
