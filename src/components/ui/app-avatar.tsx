    ...
    <div className="relative inline-block">
      ...
      >
        {image ? (
          ...
          <AvatarFallback className={cn('text-white font-bold', bg)}>{initials}</AvatarFallback>
        )}
      </UiAvatar>

      {/* Overlay actions */}
      {(preview || hoverAction) && (
        <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/40 opacity-0 hover:opacity-100 transition rounded-full">
          {preview && image && (
            <PreviewImageDialog
              name={name}
              image={image}
